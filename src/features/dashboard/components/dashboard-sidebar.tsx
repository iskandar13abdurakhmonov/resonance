...
import {Headphones, LucideIcon, Settings, Volume2} from "lucide-react";
...
function NavSection({ label, items, pathname }: NavSectionProps) {
    return (
        <SidebarGroup>
            {label && (
                <SidebarGroupLabel
                    className="text-[13px] uppercase text-muted-foreground"
                >
                    {label}
                </SidebarGroupLabel>
            )}
                    ...
                    {items.map((item, index) => (
                        <SidebarMenuItem key={index}>
                            <SidebarMenuButton
                                asChild={!!item.url}
                                isActive={
                                    item.url ? item.url === '/' ? pathname === '/' : pathname.startsWith(item.url) : false
                                }
    ...
    return (
        <Sidebar collapsible="icon">
                        ...
                        <OrganizationSwitcher
                            ...
                            appearance={{
                                elements: {
                                    ...
                                    organizationSwitcherTrigger: "w-full! justify-between! bg-white! border! border-border! rounded-md! pl-1! pr-2! py-1! gap-3! group-data-[collapsible=icon]:w-auto! group-data-[collapsible=icon]:p-1!",
            ...
            <SidebarContent>
                ...
                <NavSection items={othersMenuItems} pathname={pathname} label="Others"/>
            </SidebarContent>
            <div className="border-b border-dashed border-border"/>
